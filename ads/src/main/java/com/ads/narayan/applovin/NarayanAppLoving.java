...
public class NarayanAppLoving {
    private static final String TAG = "AppLovin";
    private static NarayanAppLoving instance;
...
//    private final Map<String, AppOpenAd> appOpenAdMap = new HashMap<>();
    ...
    public static NarayanAppLoving getInstance() {
        if (instance == null) {
            instance = new NarayanAppLoving();
    ...
    public void onCheckShowSplashWhenFail(Activity activity, AppLovinCallback callback, int timeDelay) {
        if (NarayanAppLoving.getInstance().getInterstitialSplash() != null && !NarayanAppLoving.getInstance().isShowLoadingSplash) {
            new Handler(activity.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (NarayanAppLoving.getInstance().getInterstitialSplash().isReady()) {
                        Log.i(TAG, "show ad splash when show fail in background");
                        NarayanAppLoving.getInstance().onShowSplash(activity, callback);
                    } else {
                        callback.onAdClosed();
                    }
                }
